...
## 📋 角色身份识别规则（必须遵守）
在处理新增记忆时，你必须首先识别每条记忆的发送者身份：

**角色类型：**
- user：真实用户（御主/Merlin/慕容/龙龙）
- assistant：AI Agent（太公望/卫宫切嗣/苏格拉底/BB/所罗门 等）
- system：系统消息

**识别方法：**
优先使用 L1 记录中的 sender_label 字段判断。sender_label 格式为"发送者身份（发送者名称）"，如"user（Merlin）"表示 Merlin 发送的消息，"assistant（BB）"表示 BB 发送的消息。若无 sender_label，则根据会话上下文判断。

**重要：scene 描述必须使用正确的 sender_label**
生成 scene 名称和内容时，必须使用实际的发送者身份，不得将 user 误标为 assistant 或将 assistant 误标为 user。例如：
- "我（assistant）与 user（Merlin）讨论..." 表示 AI 与 Merlin 的对话
- "我（user）与 assistant（BB）协作..." 表示 Merlin 与 BB 的对话
...
${existingSceneFiles.map((f) => `- String.fromCharCode(96)${f}String.fromCharCode(96)`).join("\n")}
`
    : `### 📁 已有场景文件清单
（当前无已有场景文件）
`}
## ⛔ 文件操作约束（必须严格遵守）
1. **所有文件操作使用相对文件名**（如 String.fromCharCode(96)技术研究-Rust学习.mdString.fromCharCode(96)），当前工作目录已设为场景文件目录
2. **read_file 只能读取上面"已有场景文件清单"中列出的文件**，禁止猜测或编造不在清单中的文件名
3. **创建新场景文件时**，直接使用文件名，如 String.fromCharCode(96)新场景名.mdString.fromCharCode(96)
4. **场景文件支持 replace_in_file**。对于局部更新（如只更新某个章节或 META 字段），可以使用 String.fromCharCode(96)replace_in_fileString.fromCharCode(96) 进行精确替换。对于大范围重写或结构性变更，建议使用 String.fromCharCode(96)read_fileString.fromCharCode(96) + String.fromCharCode(96)write_to_fileString.fromCharCode(96) 整体重写。
5. **场景索引和系统配置由工程系统自动维护**，你只需专注于操作 String.fromCharCode(96).mdString.fromCharCode(96) 场景文件
...
需要时使用 String.fromCharCode(96)read_fileString.fromCharCode(96) 工具读取完整场景文件内容
...
3. **CREATE（新建）**【最后手段】: 
   - **前提条件**：当前场景总数未达上限
   - **CREATE 前的强制验证**：必须先用 String.fromCharCode(96)read_fileString.fromCharCode(96) 检查至少 2 个最相似的现有场景，确认新记忆确实无法融入后才能 CREATE。跳过验证直接 CREATE 是被禁止的
   - 如果话题是全新的且与现有内容区分度高，可以创建新 Block
   - **每次批处理最多新增 1 个场景**

**示例 A：新记忆整合进已有 block（UPDATE - 原地更新）**
**具体操作步骤（工具调用）**：
1. String.fromCharCode(96)read_file('Python后端开发.md')String.fromCharCode(96) → 获取已有内容 A
2. 分析新记忆 + 已有内容 A → 整合生成新内容 B（String.fromCharCode(96)heat = 旧heat + 1String.fromCharCode(96)）
3. String.fromCharCode(96)write_to_file('Python后端开发.md', B)String.fromCharCode(96) → **整体重写该场景文件**
   或 String.fromCharCode(96)replace_in_file('Python后端开发.md', old_section, new_section)String.fromCharCode(96) → **局部更新某部分**

合并多个 block 的逻辑：
**具体操作步骤（工具调用）**：
1. String.fromCharCode(96)read_file('Python后端开发.md')String.fromCharCode(96) → 获取内容 A
2. String.fromCharCode(96)read_file('Go后端开发.md')String.fromCharCode(96) → 获取内容 B
3. 整合 A + B + 新记忆 → 生成新内容 C（String.fromCharCode(96)heat = heatA + heatB + 1String.fromCharCode(96)）
4. String.fromCharCode(96)write_to_file('后端开发技术栈.md', C)String.fromCharCode(96) → 创建新文件，写入合并后的完整内容
5. String.fromCharCode(96)write_to_file('Python后端开发.md', '')String.fromCharCode(96) → **清空旧文件 A（标记删除）**
6. String.fromCharCode(96)write_to_file('Go后端开发.md', '')String.fromCharCode(96) → **清空旧文件 B（标记删除）**
...
String.fromCharCode(96)String.fromCharCode(96)String.fromCharCode(96)markdown
-----META-START-----
...
**执行文件操作**（必须使用工具）：
   - 使用 String.fromCharCode(96)read_fileString.fromCharCode(96) 读取需要更新的场景文件
   - 使用 String.fromCharCode(96)write_to_fileString.fromCharCode(96) 创建新文件或**整体重写**已有场景文件
   - 使用 String.fromCharCode(96)replace_in_fileString.fromCharCode(96) 对场景文件进行**局部更新**（如只更新某个章节）
   - **删除文件**：使用 String.fromCharCode(96)write_to_file(filename, '')String.fromCharCode(96) 将文件内容清空（系统会自动清理空文件，禁止使用其他删除方式）`;
}
